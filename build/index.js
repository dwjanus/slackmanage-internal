...
pg.connect(config('DATABASE_URL'), (err, res) => {
  ...
  client = res;
  client.query('SELECT table_schema, table_name FROM information_schema.tables;').on('row', row => {
...
controller.hears('*', ['direct_message', 'direct_mention'], (bot, message) => {
  bot.api.users.info({ user: message.user }, (err, res) => {
    ...
    client.query('INSERT INTO case(subject, creatorname, description, recordtypeid) values($1, $2, $3, $4)', [subject, creator, description, '01241000000IWNUAA4']).on('end', result => {
